...
async function sendStatusEmail(
  ...
  try {
    ...
    if (!res.ok) console.error('Status email failed:', await res.json().catch(() => ({})))
  } catch (e) {
    console.error('sendStatusEmail error:', e instanceof Error ? e.message : e)
  }
}
...
function CampaignModal({ campaign, onClose, onUpdate }: {
  ...
  async function updateStatus(status: 'approved' | 'rejected') {
    ...
    await supabase.from('campaigns').update({ status, verified: status === 'approved' }).eq('id', campaign.id)
    ...
    <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-gray-900 border border-white/10 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex items-start justify-between p-6 border-b border-white/10">
          ...
          <button onClick={onClose} className="text-white/30 hover:text-white text-2xl leading-none ml-4">x</button>
        </div>
        ...
        <div className="p-6 flex flex-col gap-5">
          <div className="bg-white/5 rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div><div className="text-white/30 text-xs mb-0.5">Goal</div><div className="text-primary font-black">GH₵{campaign.goal_amount?.toLocaleString()}</div></div>
                ...
                {campaign.id_front_url && (
                  <div>
                    ...
                    <a href={campaign.id_front_url} target="_blank" rel="noopener noreferrer" className="block group">
                      <img src={campaign.id_front_url} alt="ID document"
          ...
          <div>
            <label className="text-white/40 text-xs font-bold uppercase tracking-wider block mb-2">
              Note to fundraiser <span className="text-white/20 font-normal normal-case">(included in their email)</span>
            </label>
            <textarea value={note} onChange={e => setNote(e.target.value)} rows={3}
              placeholder="e.g. Your ID photo is too blurry. Please resubmit with a clear, well-lit photo of your Ghana Card."
          ...
          <div className="bg-primary/10 border border-primary/20 rounded-xl px-4 py-3 text-xs text-primary/80">
            An email will be sent to the fundraiser when you approve or reject their campaign.
          </div>
        </div>
        ...
        {campaign.status === 'pending' ? (
          <div className="flex gap-3 p-6 border-t border-white/10">
            ...
            <button onClick={() => updateStatus('approved')} disabled={loading}
              ...
              {loading ? 'Saving...' : 'Approve and go live'}
            </button>
          </div>
        ) : (
          <div className="flex gap-3 p-6 border-t border-white/10">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/5 text-left">
                  {['Campaign', 'Fundraiser', 'Tier', 'Goal', 'Status', 'Date', 'Action'].map(h => (
              ...
              <tbody className="divide-y divide-white/5">
                {filtered.map(c => (
                  <tr key={c.id} className="hover:bg-white/[0.03] transition-all group">
                    ...
                    <td className="px-5 py-4 font-nunito font-black text-primary">GH₵{c.goal_amount?.toLocaleString()}</td>
                    <td className="px-5 py-4">
