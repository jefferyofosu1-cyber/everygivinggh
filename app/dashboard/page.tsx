...
type Tab     = 'overview' | 'campaigns' | 'donations' | 'withdraw'
type WStep   = 'form' | 'confirm' | 'sent'
      ...
      {data.map((d, i) => (
        <div key={i} className="flex-1 flex flex-col items-center gap-1 group relative">
          <div className="absolute -top-7 left-1/2 -translate-x-1/2 bg-navy text-white text-xs px-2 py-1 rounded-lg opacity-0 group-hover:opacity-100 transition-all whitespace-nowrap pointer-events-none z-10">
            GH₵{d.value.toLocaleString()}
          </div>
          <div className="w-full bg-primary rounded-t-md transition-all duration-700 hover:bg-primary-dark"
...
function CampaignCard({ campaign }: { campaign: Campaign }) {
  const pct = campaign.goal_amount
    ...
    : 0
  const s = STATUS[campaign.status] ?? STATUS.pending
        ...
        <div className="flex gap-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="text-xs text-gray-400 mb-3">
              {campaign.category} · {new Date(campaign.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
            </div>
            <div className="flex gap-4 text-sm">
              <div><div className="font-black text-primary text-base">GH₵{(campaign.raised_amount ?? 0).toLocaleString()}</div><div className="text-xs text-gray-400">raised</div></div>
              <div><div className="font-black text-navy text-base">GH₵{(campaign.goal_amount ?? 0).toLocaleString()}</div><div className="text-xs text-gray-400">goal</div></div>
              <div><div className="font-black text-navy text-base">{campaign.donor_count}</div><div className="text-xs text-gray-400">donors</div></div>
            </div>
          </div>
        </div>
        ...
        {campaign.status === 'approved' && (
          <div className="mt-4 flex gap-2">
            <Link href={`/campaigns/${campaign.id}`}
              className="flex-1 text-center py-2.5 bg-primary text-white font-bold text-xs rounded-xl hover:bg-primary-dark transition-all">
              View campaign
            </Link>
            <button onClick={() => {
            ...
            }} className="px-4 py-2.5 border-2 border-gray-100 hover:border-primary/30 text-gray-500 hover:text-primary font-bold text-xs rounded-xl transition-all">
              Share
            </button>
          </div>
        )}
        {campaign.status === 'pending' && (
          <div className="mt-4 bg-amber-50 rounded-xl px-4 py-3 text-xs text-amber-700">
            Our team is reviewing your campaign. You will receive an email once a decision is made.
          </div>
        )}
        {campaign.status === 'rejected' && (
          <div className="mt-4 bg-red-50 rounded-xl px-4 py-3 text-xs text-red-600">
            Your campaign was not approved. Check your email for the reason, then start a new corrected campaign.
          </div>
        )}
      </div>
    </div>
...
function WithdrawModal({ available, profile, onClose }: {
  ...
  async function handleConfirm() {
    setLoading(true)
    // Hubtel payout API will go here
        ...
        {wStep === 'confirm' && (
          <div className="p-6">
            ...
            <div className="bg-gray-50 rounded-2xl p-5 mb-5 flex flex-col gap-3 text-sm">
              <div className="flex justify-between"><span className="text-gray-500">Amount requested</span><span className="font-bold text-navy">GH₵{amtNum.toFixed(2)}</span></div>
              <div className="flex justify-between"><span className="text-gray-500">Transfer fee</span><span className="text-gray-500">-GH₵{fee}</span></div>
              <div className="h-px bg-gray-200" />
              <div className="flex justify-between"><span className="font-black text-navy">You receive</span><span className="font-black text-primary text-lg">GH₵{receive.toFixed(2)}</span></div>
              <div className="h-px bg-gray-200" />
              <div className="flex justify-between">
                <span className="text-gray-500">To</span>
                <span className="font-bold text-navy">
                  {method === 'momo'
                    ? `${profile?.momo_network?.toUpperCase() ?? 'MoMo'} · ${profile?.momo_number ?? '-'}`
                    : `${profile?.bank_name ?? 'Bank'} · ${profile?.bank_account ?? '-'}`}
                </span>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setWStep('form')} className="flex-1 py-3.5 border-2 border-gray-200 text-gray-500 font-bold rounded-full text-sm">Back</button>
              <button onClick={handleConfirm} disabled={loading}
                className="flex-[2] py-3.5 bg-primary hover:bg-primary-dark text-white font-nunito font-black rounded-full text-sm disabled:opacity-60 transition-all">
        ...
        {wStep === 'form' && (
          <div className="p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={onClose} className="text-gray-300 hover:text-gray-500 text-2xl leading-none">x</button>
            </div>
            ...
            <div className="mb-5">
              ...
              {amtNum > 0 && (
                <div className="mt-2 text-xs text-gray-400">
                  Fee: GH₵{fee} · You receive: <span className="text-primary font-bold">GH₵{receive.toFixed(2)}</span>
                </div>
              )}
            </div>
            ...
            <button disabled={amtNum < 10 || amtNum > available} onClick={() => setWStep('confirm')}
            ...
            <p className="text-xs text-gray-300 text-center mt-3">Minimum withdrawal: GH₵10 · Arrives within 24 hours</p>
          </div>
        )}
      </div>
    </div>
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const rawCamps = (camps ?? []) as Omit<Campaign, 'donor_count'>[]
      const enriched = await Promise.all(rawCamps.map(async c => {
        const { count } = await supabase
          .from('donations')
          .select('*', { count: 'exact', head: true })
          .eq('campaign_id', c.id)
          .eq('status', 'success')
        return { ...c, donor_count: count ?? 0 } as Campaign
      }))
  ...
  const last14 = Array.from({ length: 14 }, (_, i) => {
    ...
    const dayTotal = donations
      .filter(don => new Date(don.created_at).toDateString() === d.toDateString())
      .reduce((s, don) => s + (don.amount ?? 0), 0)
    return { label, value: dayTotal }
  })

  const weeklyTotals = Array.from({ length: 8 }, (_, w) => {
    const start = new Date()
    start.setDate(start.getDate() - (7 * (7 - w) + 7))
    const end = new Date()
    end.setDate(end.getDate() - (7 * (7 - w)))
    return donations
      .filter(d => { const t = new Date(d.created_at); return t >= start && t < end })
  ...
  const firstName = profile?.full_name?.split(' ')[0] ?? userEmail.split('@')[0] ?? 'there'
          ...
          <div className="max-w-5xl mx-auto">
            <div className="flex items-center justify-between flex-wrap gap-4 mb-8">
              ...
              <div className="flex gap-2">
                ...
                <Link href="/create"
                  className="bg-primary hover:bg-primary-dark text-white font-nunito font-black px-5 py-2.5 rounded-full text-sm transition-all hover:-translate-y-0.5 shadow-lg shadow-primary/30 whitespace-nowrap">
                  + New campaign
                </Link>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {[
                { label: 'Total raised',    value: `GH₵${totalRaised.toLocaleString()}`,    sub: `from ${donations.length} donations`,                        trend: weeklyTotals },
                { label: 'Total donors',    value: totalDonors.toLocaleString(),             sub: `avg GH₵${avgDonation} per donation`,                        trend: null         },
                ...
                { label: 'Available',       value: `GH₵${availableBalance.toLocaleString()}`, sub: 'ready to withdraw',                                        trend: null         },
              ].map((s, i) => (
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="text-right">
                    <div className="font-black text-primary text-xl">GH₵{last14.reduce((s, d) => s + d.value, 0).toLocaleString()}</div>
                      ...
                      <div className="flex flex-col gap-3">
                        {topCampaigns.map((c, i) => {
                          const pct = c.goal_amount
                            ? Math.min(Math.round((c.raised_amount / c.goal_amount) * 100), 100)
                            : 0
                          return (
                            <div key={c.id} className="flex items-center gap-3">
                              ...
                              <div className="text-xs font-black text-primary flex-shrink-0">GH₵{(c.raised_amount ?? 0).toLocaleString()}</div>
                            </div>
                          )
                        })}
                      </div>
                ...
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                  ...
                  <div className="flex flex-col gap-3 text-sm">
                    {([
                      ...
                      ['Payout method', profile?.payout_method === 'momo' ? 'Mobile Money' : profile?.payout_method === 'bank' ? 'Bank' : null],
                  ...
                  {!profile?.momo_number && !profile?.bank_account && (
                    <div className="mt-4 bg-amber-50 border border-amber-100 rounded-xl px-3 py-2.5 text-xs text-amber-700">
                      Set your payout details before withdrawing funds.
                    </div>
                  )}
                </div>
              </div>
                        ...
                        <div key={c.id} className="flex items-center gap-3">
                          <div className="flex-1 min-w-0">
                            <div className="flex justify-between text-xs mb-1">
                              ...
                              <span className="text-primary font-black ml-2">GH₵{(c.raised_amount ?? 0).toLocaleString()}</span>
                            </div>
                  ...
                  : donations.map((d, i) => (
                    <div key={d.id} className={`flex items-center gap-4 px-5 py-4 ${i < donations.length - 1 ? 'border-b border-gray-50' : ''}`}>
                      ...
                      <div className="text-right flex-shrink-0">
                        <div className="font-black text-primary text-sm">GH₵{(d.amount ?? 0).toLocaleString()}</div>
            ...
            <div className="flex flex-col gap-4 max-w-lg">
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
                ...
                <div className="flex flex-col gap-3 mb-6">
                  {[
                    ['Payout method', profile?.payout_method === 'bank' ? 'Bank transfer' : 'Mobile Money'],
                    profile?.momo_number ? ['MoMo number', `${profile.momo_network?.toUpperCase() ?? ''} · ${profile.momo_number}`] : null,
                    profile?.bank_account ? ['Bank account', `${profile.bank_name ?? 'Bank'} · ${profile.bank_account}`] : null,
                    ['Transfer fee', '2% + GH₵0.25 per withdrawal'],
                    ['Arrival time', 'Within 24 hours'],
                ...
                {!profile?.momo_number && !profile?.bank_account ? (
                  <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 mb-4">
                    <div className="font-bold text-amber-800 text-sm mb-1">Payout details not set</div>
                    <div className="text-amber-700 text-xs mb-3">Add your MoMo number or bank account before withdrawing.</div>
                  ...
                  <button onClick={() => setShowWithdraw(true)}
                    ...
                    Withdraw GH₵{availableBalance.toLocaleString()}
                  </button>
                )}
                <p className="text-xs text-gray-300 text-center mt-3">Minimum withdrawal: GH₵10</p>
              </div>
            </div>
          )}
