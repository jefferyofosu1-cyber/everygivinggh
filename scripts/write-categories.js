      ...
      <nav style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'0 32px', height:60, background:'#fff', borderBottom:'1px solid #E8E4DC', position:'sticky', top:0, zIndex:200 }}>
        ...
        <div style={{ display:'flex', gap:4, alignItems:'center' }}>
          {([
            ...
            ['/fundraising-categories','Categories'],
            ['/fees','Fees'],
          ] as [string,string][]).map(([href, label]) => (
            <Link key={href} href={href} style={{ fontSize:13, fontWeight:href==='/fundraising-categories'?600:500, color:href==='/fundraising-categories'?'#0A6B4B':'#4A4A44', padding:'7px 12px', borderRadius:6 }}>{label}</Link>
          ))}
        </div>
