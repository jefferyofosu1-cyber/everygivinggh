        ...
        <div className="flex items-start justify-between p-6 border-b border-white/10">
          <div>
            ...
            <div className="flex items-center gap-2 flex-wrap">
              <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${TIER_BADGE[campaign.verification_tier]}`}>{campaign.verification_tier}</span>
        ...
        <div className="p-6 flex flex-col gap-5">
          ...
          <div className="bg-white/5 rounded-xl p-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm mb-3">
              <div><div className="text-white/30 text-xs mb-0.5">Ghana Card ID</div><div className="text-white font-mono font-bold">{campaign.id_number || '—'}</div></div>
              <div><div className="text-white/30 text-xs mb-0.5">NIA Verified</div><div className={`font-bold text-sm ${campaign.nia_verified ? 'text-primary' : 'text-amber-400'}`}>{campaign.nia_verified ? '✓ Verified' : '⏳ Pending'}</div></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              {campaign.id_front_url && (
                <a href={campaign.id_front_url} target="_blank" rel="noopener noreferrer"
                  ...
                  <div className="text-white/60 text-xs font-bold">ID Front</div>
                  <div className="text-primary text-xs mt-0.5">View document →</div>
                </a>
              )}
              {campaign.id_back_url && (
                <a href={campaign.id_back_url} target="_blank" rel="noopener noreferrer"
                  className="bg-white/5 border border-white/10 rounded-xl p-3 text-center hover:border-primary/40 transition-all">
                  <div className="text-2xl mb-1">🪪</div>
                  <div className="text-white/60 text-xs font-bold">ID Back</div>
                  <div className="text-primary text-xs mt-0.5">View document →</div>
                </a>
              )}
              {campaign.selfie_url && (
                <a href={campaign.selfie_url} target="_blank" rel="noopener noreferrer"
                  ...
                  <div className="text-primary text-xs mt-0.5">View photo →</div>
                </a>
              )}
            </div>
            {!campaign.id_front_url && !campaign.id_back_url && (
              <div className="text-white/20 text-xs text-center py-3">No documents uploaded yet</div>
            )}
          </div>
          ...
          {/* Admin note */}
          <div>
            <label className="text-white/40 text-xs font-bold uppercase tracking-wider block mb-2">Admin note (optional)</label>
            <textarea value={note} onChange={e => setNote(e.target.value)} rows={2} placeholder="Internal note — not shown to fundraiser"
  ...
  return (
    <div>
      ...
      <div className="bg-gray-900 border border-white/5 rounded-2xl overflow-hidden">
        {loading ? (
          <div className="p-8 text-center text-white/30 text-sm">Loading campaigns…</div>
        ) : filtered.length === 0 ? (
              ...
              <tbody className="divide-y divide-white/5">
                {filtered.map(c => (
                  <tr key={c.id} className="hover:bg-white/3 transition-all group">
                    ...
                    <td className="px-5 py-4">
                      <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${TIER_BADGE[c.verification_tier] || ''}`}>
                        {c.verification_tier}
                      </span>
                    </td>
