...
export default function CampaignPage() {
  ...
  const fundraiserReceives = rawAmount > 0 ? +(rawAmount - fee).toFixed(2) : 0
  const tipAmount = tipEnabled ? (tipMode === 'custom' ? +(parseFloat(tipCustom) || 0).toFixed(2) : tipValue) : 0
  const total = +(rawAmount + tipAmount).toFixed(2)

  const shareUrl = typeof window !== 'undefined' ? window.location.href : `https://everygiving.org/campaigns/${campaign.id}`
      ...
      <div className="min-h-screen bg-gradient-to-br from-primary-light via-white to-blue-50 flex items-center justify-center px-5 py-12">
        <div className="max-w-md w-full text-center">
          ...
          <p className="text-gray-500 text-sm mb-1">
            Your donation of <strong className="text-primary">₵{rawAmount.toFixed(2)}</strong> to <strong>{campaign.title}</strong> is being processed.
          </p>
          {tipAmount > 0 && (
            <p className="text-xs text-gray-400 mb-6">Plus ₵{tipAmount.toFixed(2)} to help keep EveryGiving free. Thank you. 💚</p>
          )}
      ...
      <main className="bg-gray-50 min-h-screen">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 py-8">
          ...
          <div className="grid lg:grid-cols-5 gap-8">
            ...
            {/* ── LEFT col (3/5): Campaign info + donors ── */}
                ...
                <div className="mb-6">
                  <div className="flex items-end gap-2 mb-2">
                    ...
                    <span className="text-gray-400 text-sm pb-1">
                      raised of ₵{(campaign.goal_amount || 0).toLocaleString()} goal
                    </span>
                  </div>
              ...
              {donations.length > 0 && (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sm:p-8">
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="font-nunito font-black text-navy text-lg">
            ...
            {/* ── RIGHT col (2/5): Sticky donate form ── */}
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 sticky top-20">
                ...
                <form onSubmit={handleDonate} className="flex flex-col gap-4">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-4 gap-1.5 mb-2">
                      {[20, 50, 100, 200].map(p => (
                        <button key={p} type="button"
                          ...
                          className={`text-xs font-bold py-2.5 rounded-xl border-2 transition-all ${form.amount === String(p) ? 'border-primary bg-primary text-white' : 'border-gray-100 text-gray-500 bg-gray-50 hover:border-primary/40'}`}>
                          ₵{p}
                        </button>
                      ))}
                    </div>
                    ...
                    {rawAmount > 0 && (
                      <div className="mt-1.5 bg-gray-50 rounded-lg px-3 py-2 text-xs text-gray-400">
                        Fee: ₵{fee.toFixed(2)} (2% + ₵0.25) · Fundraiser gets <strong className="text-primary">₵{fundraiserReceives.toFixed(2)}</strong>
                      </div>
                    )}
                  </div>
                          ...
                          <div className="flex gap-1.5">
                            {TIP_PRESETS.map(v => (
                              <button key={v} type="button"
                                ...
                                className={`flex-1 text-xs font-bold py-2 rounded-xl border-2 transition-all ${tipMode === 'preset' && tipValue === v ? 'border-primary bg-primary text-white' : 'border-primary/20 text-primary bg-white hover:border-primary/50'}`}>
                                ₵{v}
                              </button>
                            ))}
                            <button type="button"
                              onClick={() => setTipMode('custom')}
                              className={`flex-1 text-xs font-bold py-2 rounded-xl border-2 transition-all ${tipMode === 'custom' ? 'border-primary bg-primary text-white' : 'border-primary/20 text-primary bg-white hover:border-primary/50'}`}>
                              Other
                            </button>
                          </div>
                          {tipMode === 'custom' && (
                    ...
                    <div className="grid grid-cols-3 gap-1.5">
                      {['MTN MoMo', 'Vodafone', 'AirtelTigo'].map(m => (
                        <button key={m} type="button" onClick={() => setForm(p => ({ ...p, method: m }))}
                          className={`text-xs font-bold py-2.5 rounded-xl border-2 transition-all ${form.method === m ? 'border-primary bg-primary-light text-primary-dark' : 'border-gray-100 text-gray-500 bg-gray-50 hover:border-primary/30'}`}>
                          {m}
                        </button>
                      ))}
                    </div>
                  ...
                  {rawAmount > 0 && (
                    <div className="bg-gray-50 border border-gray-100 rounded-xl px-4 py-3 text-sm">
                      <div className="flex justify-between text-gray-400 mb-1 text-xs">
                        <span>Donation</span><span>₵{rawAmount.toFixed(2)}</span>
                      </div>
                      {tipAmount > 0 && (
                        <div className="flex justify-between text-gray-400 mb-1 text-xs">
                          <span>Tip to EveryGiving</span><span>₵{tipAmount.toFixed(2)}</span>
                        </div>
                      )}
                  ...
                  <button type="submit" disabled={donating || !rawAmount}
                    ...
                    {donating ? 'Processing…' : rawAmount > 0 ? `Donate ₵${total.toFixed(2)} →` : 'Donate →'}
                  </button>
                  <p className="text-xs text-gray-300 text-center">
                    Secure · MoMo prompt will appear on your phone
                  </p>
                </form>
              </div>
            </div>
