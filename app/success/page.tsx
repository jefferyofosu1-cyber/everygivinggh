...
export default function SuccessPage() {
  ...
  const shareUrl = campaign ? `https://everygivinggh.vercel.app/campaigns/${campaignId}` : 'https://everygivinggh.vercel.app'
    ...
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-primary-light via-white to-blue-50 flex items-center justify-center px-5 py-12">
        <div className="max-w-lg w-full text-center">
          ...
          <p className="text-gray-500 mb-2">Your donation of <strong className="text-primary font-black">₵{amount || '...'}</strong> has been received</p>
          ...
          {campaign && (
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-6 mx-auto max-w-sm">
              ...
              <div className="flex justify-between text-xs mt-1.5">
                ...
                <span className="text-primary font-bold">{Math.round((campaign.raised_amount / campaign.goal_amount) * 100)}%</span>
              </div>
            </div>
          )}

          {/* Share section — THE most important part */}
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
            <div className="font-nunito font-black text-navy text-lg mb-2">🚀 Now help them raise more!</div>
            <p className="text-sm text-gray-400 mb-5">
              Campaigns that get shared raise <strong className="text-navy">4× more money</strong>. Share this with your network and make a bigger impact.
            </p>

            <div className="flex flex-col gap-3">
              <a href={whatsappUrl} target="_blank" rel="noopener noreferrer"
                className="flex items-center justify-center gap-3 py-3.5 bg-[#25D366] hover:bg-[#20BD5C] text-white font-nunito font-black rounded-xl transition-all hover:-translate-y-0.5 shadow hover:shadow-lg text-sm">
              ...
              <a href={twitterUrl} target="_blank" rel="noopener noreferrer"
                ...
                <svg viewBox="0 0 24 24" className="w-4 h-4 fill-current"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.744l7.73-8.835L1.254 2.25H8.08l4.253 5.622 5.91-5.622zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
                Share on X (Twitter)
              </a>
          ...
          <div className="flex gap-3 justify-center">
            {campaign && (
              <Link href={`/campaigns/${campaignId}`} className="px-5 py-2.5 border-2 border-gray-200 hover:border-primary hover:text-primary text-gray-600 font-bold rounded-xl transition-all text-sm">
                View campaign
              </Link>
            )}
            <Link href="/campaigns" className="px-5 py-2.5 bg-primary hover:bg-primary-dark text-white font-bold rounded-xl transition-all text-sm">
              Discover more campaigns
            </Link>
          </div>

          <p className="text-xs text-gray-400 mt-6">
            A receipt has been sent to your email. Questions? <Link href="#" className="text-primary underline">Contact us</Link>
          </p>
        </div>
      </div>
    </>
