...
export default function CampaignPage() {
  ...
  const handleDonate = async () => {
    ...
    const { data: donation } = await supabase.from('donations').insert({
      ...
      message, payment_method: 'paystack', status: 'pending'
    }).select().single()
    ...
    if (!paystackKey || paystackKey.includes('REPLACE')) {
      ...
      await supabase.from('campaigns').update({ raised_amount: (campaign?.raised_amount || 0) + finalAmount }).eq('id', id)
      toast.success(`₵${finalAmount} donation recorded! Thank you ${donorName} 🎉`)
      setShowModal(false)
      fetchCampaign()
    } else {
